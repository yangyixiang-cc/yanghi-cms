...
async function transformDirective(code, transformer2, s, ctx, sfcPath) {
  ...
  for (const { rawName, replace } of results) {
    debug3(`| ${rawName}`);
    const name = `${_chunk44ONIEX7js.DIRECTIVE_IMPORT_PREFIX}${_chunk44ONIEX7js.pascalCase.call(void 0, rawName)}`;
    ctx.updateUsageMap(sfcPath, [name]);
    const directive = await ctx.findComponent(name, "directive", [sfcPath]);
    if (!directive)
      continue;
    const varName = `__unplugin_directives_${no}`;
    s.prepend(`${_chunk44ONIEX7js.stringifyComponentImport.call(void 0, _chunkBTQOTIPQjs.__spreadProps.call(void 0, _chunkBTQOTIPQjs.__spreadValues.call(void 0, {}, directive), { as: varName }), ctx)};
`);
    no += 1;
    replace(varName);
  }
...
var Context = class {
  ...
  transform(code, id) {
    const { path, query } = _chunk44ONIEX7js.parseId.call(void 0, id);
  ...
  setupWatcher(watcher) {
    ...
    watcher.on("unlink", (path) => {
      if (!_chunk44ONIEX7js.matchGlobs.call(void 0, path, globs))
        return;
...
var unplugin_default = _unplugin.createUnplugin.call(void 0, (options = {}) => {
  ...
  const api = {
    ...
    stringifyImport(info) {
      return _chunk44ONIEX7js.stringifyComponentImport.call(void 0, info, ctx);
    }
  };
  return {
    ...
    async transform(code, id) {
      if (!_chunk44ONIEX7js.shouldTransform.call(void 0, code))
        return null;
      try {
